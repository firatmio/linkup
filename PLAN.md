...
- **Biriktirme (Faz 7.5'te eklendi).** Bildirim başına bir toast üretmek yetmiyordu: karşı taraf art arda beş mesaj yazdığında Windows'un kuyruğuna beş toast giriyor ve bunlar kullanıcı uygulamaya döndükten sonra bile teker teker düşmeye devam ediyordu — toast teslim edildikten sonra geri alınamıyor (kullanılan kütüphane `tag`/`history` API'sini dışa açmıyor). Bu yüzden gösterim ~1,5 sn geciktiriliyor, o sürede aynı konudan gelenler tek bildirimde toplanıyor ve **odak kararı gecikmenin sonunda** veriliyor: kullanıcı bu arada uygulamaya döndüyse bildirim hiç gösterilmiyor. Gösterimden sonra aynı konu için 20 sn boyunca yenisi çıkmıyor; o sürede birikenler tek özet oluyor. Konu anahtarı cihaz kimliği + bildirim türü.
...
- Native bildirimler Faz 7'de öne alındı (kullanıcı isteği): mesaj ve dosya geldiğinde, pencere odakta DEĞİLSE gösteriliyor. Faz 8'in ilk işi olan **tıklama spike'ı da Faz 7'de tamamlandı** — eklentinin masaüstünde olay yayınlamadığı doğrulandı, `tauri-winrt-notification` ile çözüldü ve tıklama yönlendirmesi Windows'ta çalışıyor (§2.10). Faz 8'e bu maddeden yalnızca macOS/Linux yönlendirmesi kaldı.
...
**K11 — Devam ettirilemeyen bir transfer "duraklatıldı" değil "başarısız" sayılır.**
Bağlantı koptuğunda yarım transferler önce `paused` işaretleniyordu; `.part` dosyası durduğu için bu doğru görünüyordu. Değildi: yeniden bağlanınca teklifi tekrar gönderen bir mekanizma yok (resume Faz 11'e ertelendi), dolayısıyla o kayıt hiçbir zaman ilerlemiyor. Sonuç, aktif aktarımlar listesinde sonsuza kadar duran — çoğu zaman %100 dolu görünen — hayalet satırlardı; uygulamayı yeniden başlatmak da temizlemiyordu, çünkü açılıştaki `fail_stale` yalnızca `active`/`pending` kayıtlara bakıyordu. Artık kopan bağlantı transferi "bağlantı kesildi" hatasıyla sonlandırıyor. `bytes_done` ve `.part` dosyası korunuyor: resume geldiğinde kayıtlar hâlâ kaldıkları yeri biliyor, o zaman bu karar geri alınabilir.

**K12 — Windows bildirimlerinde kimlik yapı tipiyle seçilir, denemeyle değil.**
Kayıtsız bir AppUserModelID ile toast göndermek hata döndürmez; Windows bildirimi sessizce yutar. "Önce uygulamanın kendi kimliğini dene, başarısızsa PowerShell'e düş" mantığı bu yüzden çalışmadı — ilk deneme her zaman başarılı göründü ve geliştirme yapılarında bildirimler tamamen kayboldu. Kimlik artık `debug_assertions` ile seçiliyor: geliştirme yapısı kurulmadığı için PowerShell'in kayıtlı kimliğini ödünç alır (gönderen adı "Windows PowerShell" görünür, bildirim çıkar), kurulu sürüm kendi kimliğini kullanır.
